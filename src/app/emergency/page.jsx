...
export default function EmergencyPage() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    loadPack(selectedCountry)
      .then((data) => {
        setEmergencyData(data);
        localStorage.setItem("emergency.lastCountry", selectedCountry);
      })
      .catch((error) => {
  ...
  // 프리미엄 체크 (간단한 버전 - 실제로는 사용자 인증 필요)
  const isPremium = false; // TODO: 실제 프리미엄 체크 로직
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredCountries.map((country) => {
              ...
              return (
                <button
                  key={country.code}
                  onClick={() => !isLocked && setSelectedCountry(country.code)}
                  disabled={isLocked}
        ...
        <div className="p-4">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <div className="text-gray-500 mb-4">데이터를 불러올 수 없습니다.</div>
              <button
                onClick={() => setSelectedCountry("")}
